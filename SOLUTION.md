...
```mermaid
graph TB
    A[Claim Submission<br/>PDFs, Images, Forms] --> B[Layer 1: Perception Engine<br/>Multi-Modal AI Extraction]
...
flowchart TD
    ...
    B -->|PDF| C[GPT-4o Vision<br/>Text Extraction]
    B -->|Image/Photo| D[GPT-4o Vision<br/>OCR + Understanding]
    B -->|Scanned/Poor Quality| E[Enhancement Pipeline<br/>Deskew + Sharpen]
    
    E --> D
    
    C --> F[Schema Validation<br/>Pydantic]
    D --> F
    
    F --> G{All Fields Valid?}
    
    G -->|Yes| H[Calculate Confidence<br/>Per Field]
    G -->|No| I[Identify Missing<br/>or Invalid Fields]
    
    H --> J{Overall Confidence<br/>>= 90%?}
    
    J -->|Yes| K[✅ Validated Claim Object]
    J -->|No| L[⚠️ Flag for Human Review]
...
- **Vision AI:** Multi-modal models handle document extraction regardless of format (PDFs, images, scans)
...
- **Edge Case Handling:** Fallback mechanisms for challenging scenarios (handwriting, poor quality, non-English)

**Output:** Clean structured data with field-level confidence metadata


**Why This Works:**
- AI handles what it's good at: understanding unstructured data
...
- No AI decision-making yet - just data extraction
...
```mermaid
flowchart LR
    A[Claim Images] --> B[CLIP Encoder<br/>Image → 512D Vector]
    C[Claim Text] --> D[Sentence-BERT<br/>Text → 384D Vector]
...
**Technical Approach:**
- Image and text content converted to vector embeddings
- Vector database enables similarity search across historical claims
- Similarity thresholds tuned to balance detection vs false positives

**What It Catches:**
- Same accident photo submitted by multiple claimants
- Same hospital bill with altered dates/names
- Copy-pasted incident descriptions
- Template-based fraud (fraudsters reusing forms)
...
flowchart TB
    ...
    B --> C[Audit Trail Contains:<br/>• All extracted data + confidence<br/>• Policy rules applied + version<br/>• Fraud scores from all tiers<br/>• Economic calculation<br/>• Final decision + rationale<br/>• Model versions used<br/>• Timestamp]
    
    C --> D{Human Review<br/>Occurred?}
    
    D -->|No| E[Store for Compliance]
    D -->|Yes| F[Underwriter Decision]
    
    F --> G{Agrees with<br/>System?}
    
    G -->|Yes| H[Log Confirmation]
    G -->|No| I[Log Correction<br/>+ Reason]
    
    H --> J[Feedback Database]
    I --> J
    
    J --> K{Monthly<br/>Retraining Cycle}
    
    K --> L[Analyze<br/>Disagreements]
    
    L --> M[Retrain Models:<br/>• Fraud detection weights<br/>• Confidence thresholds<br/>• Risk scoring]
    
    M --> N[A/B Test<br/>New Model vs Current]
    
    N --> O{New Model<br/>Performance?}
    
    O -->|Lift > 2%<br/>AND<br/>FP Rate ≤ Target| P[Deploy New Model]
...
|-------|-----------|----------|------|
| Extraction | GPT-4o Vision API | Production | ✅ Low |
...
| Frontend | Next.js / Streamlit | Proven | ✅ Low |
...
sequenceDiagram
    ...
    API->>L1: Extract data
    L1->>L1: GPT-4o Vision processing
...
graph TB
    subgraph "INPUT"
        A[Messy Real-World Documents<br/>PDFs, Photos, Scans]
    end
    
    subgraph "LAYER 1: PERCEPTION"
        B[AI Extraction<br/>GPT-4o Vision]
        C[Schema Validation<br/>Pydantic]
    end
    
    subgraph "LAYER 2: GOVERNANCE"
        D[Policy Rules<br/>Human-Authored]
        E[Deterministic Engine<br/>Python]
    end
    
    subgraph "LAYER 3: INTELLIGENCE"
        F[Tier 1: Rules<br/>Fast Checks]
        G[Tier 2: Vectors<br/>Similarity]
        H[Tier 3: Graph<br/>Networks]
    end
    
    subgraph "LAYER 4: OPTIMIZATION"
        I[Economic Model<br/>Expected Loss]
        J[Decision Router<br/>Smart Routing]
    end
    
    subgraph "LAYER 5: LEARNING"
        K[Audit Trail<br/>Complete Log]
        L[Feedback Loop<br/>Improvement]
    end
